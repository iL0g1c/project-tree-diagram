...
class GetForceUsersHandler
{
    ...
    public async Task<List<(Int64, Int64)>> GetForceUsers(Int64 guild_id)
    {
        try
        {
            var results = new List<(Int64,Int64)>();

            using NpgsqlConnection connection = new NpgsqlConnection(connectionString);
            await connection.OpenAsync();
            string query = await File.ReadAllTextAsync("queries/get_force_users.sql");
